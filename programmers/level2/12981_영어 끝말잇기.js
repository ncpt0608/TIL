 ...
 * 영어 끝말잇기
 ...
 자신의 첫 번째 차례에 말한 tank와 같으므로 3번 사람이 
 자신의 세 번째 차례로 말을 할 때 처음 탈락자가 나오게 됩니다.
...
function solution(n, words) {
    ...
    obj[words[0]] = true;
    for (let ix = 1, ixLen = words.length; ix < ixLen; ix++) {
        wordLen = words[ix - 1].length;
        if (words[ix - 1][wordLen - 1] !== words[ix][0] || obj[words[ix]]) {
            answer = ix;
            break;
        }
        obj[words[ix]] = true;
    ...
    return answer ? [answer % n + 1, (answer / n | 0) + 1] : [0, 0]
}